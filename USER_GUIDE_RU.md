...
6. [AI Chat](#6-ai-chat)
 ...
 7. [Отчёты](#7-отчёты)
    ...
    - 7.2. AI-отчёт
    - 7.3. AI-черновик статьи (Article Draft)
...
- **AI-ассистента** на базе Ollama (локально), Groq (облачный API) или Gemini (Google API)
...
| Параметр | Описание |
|----------|----------|
| **Provider** | `Ollama (Local)` — локальный запуск моделей. `Groq (Cloud)` — облачный API. `Gemini (Google)` — Google API |
...
| **Gemini API Key** | Ключ API (получить на aistudio.google.com) |
| **Gemini Model** | Модель (список загружается после "Test Connection") |
...
### 6.2. Валидация чисел и авто-коррекция

Ответы AI, содержащие числа (OR, HR, p-value, AUC и т.д.), автоматически проверяются:

| Функция | Описание |
|---------|----------|
| **Точное совпадение** | Каждое число в ответе AI сравнивается с метриками источника (2 знака после запятой) |
| **Детекция галлюцинаций** | Все десятичные числа в тексте AI должны быть в источнике — ловит выдуманные числа |
| **Полнота CI** | HR/OR должны иметь 95% CI в том же предложении |
| **Корректность p-value** | p<0.05 → "significant", p≥0.05 → "not significant" |
| **Проверка цитат** | Числа в цитируемых предложениях проверяются по абстракту статьи |
| **Запрещённые фразы** | "almost significant", "trend toward significance" и т.д. блокируются |
| **Авто-повтор** | При ошибке AI получает промпт с коррекцией и переписывает ответ (до 2 попыток) |
| **Уведомление** | В конце ответа: `✅ Verified: X/Y numbers match` |

### 6.3. Режимы: Assistant и Coder
...
- Числа проверяются перед отображением
...
- **Внимание**: валидация чисел отключена в режиме Coder

### 6.4. Обработка ошибок

Все AI-провайдеры показывают понятные сообщения об ошибках без технических трейсбеков:

| Ошибка | Отображение |
|--------|-------------|
| Таймаут | `Error (model): Request timed out (60s)...` |
| API недоступен | `Error (model): Model temporarily unavailable...` |
| Неверный ключ | `Error (model): API key not valid...` |
| Другое | `Error (model): <понятное сообщение>` |
...
| **Section** | Полная статья, Методы, Результаты или Обсуждение |
| **Title** | Название статьи |
| **Generate Article Draft** | Создать черновик статьи через AI |

**Особенности:**
- Числа проверяются по метрикам источника с авто-повтором
- PubMed-цитаты проверяются по абстрактам статей
- Генерация по секциям (Methods, Results, Discussion, Full)

...
def _save_to_history(project_path, template, params, result):
    ...
    record = {
        ...
        "template": template,
        "title": title,
        "params": {k: v for k, v in params.items() if k != 'messages' and (not isinstance(v, (list, dict)) or len(str(v)) < 500)},
        "messages": params.get('messages') if template == 'ai_chat' else None,
...
async def generate_ai_report(req: AIReportRequest):
    ...
    # 2. Build prompt using ContextBuilder for analyses
    ...
    # Count included analyses (same logic as build_context_for_report caps at 5)
    history_path = loader.project_path / "state" / "analysis_history.json"
    if history_path.exists():
        ...
        analyses_count = min(len(counted), 5)
    else:
        analyses_count = 0

    if not analyses_context.strip() or "No analysis history" in analyses_context:
        raise HTTPException(status_code=400, detail="No analyses selected")
    ...
    prompt_lines.append("")
    prompt_lines.append(dataset_summary)

    prompt_lines.append("\n## Analyses:")
    prompt_lines.append(analyses_context)
    ...
    doc.add_paragraph(f"Analyses included: {analyses_count}")
    doc.add_paragraph()
